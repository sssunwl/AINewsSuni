"""把訊息鏡射到 Discord webhook(#n-ainews)。失敗絕不影響主流程。純 stdlib。

webhook URL 來源(依序):
  1. 環境變數 DISCORD_WEBHOOK_AINEWS(GitHub Actions secret 用這個)
  2. 環境變數 DISCORD_WEBHOOK
  3. ~/.config/sol/config.json 的 webhooks["n-ainews"](本機跑用這個)
沒設就安靜跳過。
"""
import os
import re
...
CHANNEL = "n-ainews"
ENV_VAR = "DISCORD_WEBHOOK_AINEWS"
